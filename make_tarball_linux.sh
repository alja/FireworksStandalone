...
getGCC()
{
    ...
    mkdir -p $gccd_target
	 cp -r $gccd_src/include $gccd_target
    cp -r $gccd_src/bin $gccd_target
    cp -r $gccd_src/lib64 $gccd_target/lib64
}
...
getCMSExternals()
{
    ...
    for i in boost bz2lib castor clhep dcap db4 dcap \
   	...
   	openssl pcre \
   	sigcpp  xrootd zlib xz freetype tbb md5
    do
        export i;
        ...
        if [ $? -ne 0 ]; then
            ...
            echo "cp -a `scram tool tag $i LIBDIR`/* ${extdl}"
    ...
    for i in CLHEP HepMC boost sigcpp tbb; do
       ...
       if [ $? -eq 0 ]; then
         ...
         cp -r `scram tool tag $i INCLUDE` ${tard}/external/var-inc
       fi
    done
...
usage() { echo "Usage: $0  --tar --version=<version> --dir=<destDir> --verbose --force -all/<pkgEntry>" ; exit;}
...
if [ "$pkgExternals" = true ]; then
   getExternals
fi

if [ "$pkgCmsSources" = true ]; then
   getCmsSources
   getFireworksSources
fi

if [ "$pkgCmsLibs" = true ]; then
   getCmsLibs
fi

# Is this still needed?
cp /usr/include/wchar.h $tard/external/var-inc
if [ "$pkgCmsData" = true ]; then
   getDataFiles
fi
